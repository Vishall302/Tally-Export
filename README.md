...
ETL toolkit for Tally Prime / Tally.ERP 9 using the XML HTTP API.

Export groups, ledgers, and vouchers from Tally, then run offline analysis for
classification, Duties & Taxes closure, voucher pattern detection, and per-ledger
JSON generation.

**Highlights**
- Works with any Tally company and date range.
- Supports large XML files with streaming parsers.
- Produces both XML and enriched JSON outputs.
...
**⚠ Warning:** This script executes at import time (no `if __name__ == "__main__"` guard). Do **not** `import tally_groups` as a library unless you intend to trigger a live HTTP request.
...
python split_daybook_by_final_list.py --daybook ./daybook_*.xml --ledgers ./tally_ledgers_final.xml --groups-xml ./tally_groups_final.xml
```
...
**Field resolution priority (example):**
...
`vouchers_liability_no_expense_yes.py` (and its descendants) classify each `<ENTRY>` line by cross-referencing the line's `LEDGERNAME` against the two pre-built ledger sets, then checks `ISDEEMEDPOSITIVE`:
...
### 5. Field Resolution Cascade (`vouchers_to_json_with_ledger.py`)

Tally stores the same business identifier (GSTIN, PAN, state) in multiple XML paths for historical reasons. The script checks each path in priority order, uses the first non-empty value as canonical, and records the winning path in `field_sources` — so you can trace exactly where each value came from.
...
Both `tally_groups.py` and `tally_ledger_master.py` use the same hard-coded mapping from Tally's 20 built-in primary groups to enrichment fields:
...
> **Consistency note:** If you modify this mapping in one script, you must apply the same change in the other to keep `NATURE` / `ROOTPRIMARY` consistent between group and ledger exports.

---

## JSON Output Structure

Each file in `vouchers_by_final_list_json/` has this top-level shape:
...
- XML attributes are prefixed with `@` (e.g. `@DATE`).
...
- `_lookup_error` appears in `ledger_master` when the filename stem has no matching `<LEDGER NAME="...">` in the master file.

---

## What You Get (Example Scale)

The table below shows **typical** output sizes from one full financial year export of a mid-size Indian company. Your actual numbers will vary by company size and voucher volume.


| Artifact                       | Typical size          | Notes                                         |
| ------------------------------ | --------------------- | --------------------------------------------- |
| `tally_groups_final.xml`       | ~50–100 KB            | Tally typically has 130–160 groups            |
| `tally_ledgers_final.xml`      | 5–20 MB               | Scales with number of ledgers                 |
| `daybook_*.xml`                | 50–250 MB             | Scales with voucher count                     |
| `vouchers_by_final_list/`      | Hundreds of XML files | One per target ledger after pattern filtering |
...
| --------------------------------------------------- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| `requests.exceptions.ReadTimeout` during export     | Tally timed out on a large response                                                       | Reduce date range (smaller `--start`/`--end` window) or optimise with TDL        |
| Empty or malformed XML response                     | Tally HTTP server not enabled, or wrong port                                              | Check **F12 → Advanced Configuration** in Tally; confirm port 9000               |
| `xml.etree.ElementTree.ParseError`                  | Illegal characters in Tally response survived sanitisation                                | Report the raw response and update `clean_tally_xml`                             |
| Ledger fields missing from output (e.g. no GSTIN)   | `FETCH` spec contains fields not supported by your Tally build                            | Read the comment in `tally_ledger_master.py` about trimming unknown FETCH fields |
| `_lookup_error` in JSON files                       | Ledger was renamed in Tally after the split, or filename sanitisation produced a mismatch | Re-run `split_daybook_by_final_list.py` then `vouchers_to_json_with_ledger.py`   |
| Fewer ledger files than expected                    | Pattern filter found no matching vouchers for that ledger                                 | Check that daybook date range covers relevant vouchers                           |
| Duplicate ledger name collisions (`_2`, `_3` files) | Two distinct ledger names sanitise to the same filename                                   | This is automatic and handled; both files are written correctly                  |
| Very slow daybook export                            | Large company with many monthly vouchers                                                  | Normal — each month is a separate HTTP request; 900 s timeout per chunk          |
...
- **Hardcoded endpoint:** All export scripts POST to `http://localhost:9000`. If Tally runs on a different port or host, update the URL constant in each export script.
...
- **Consistency requirement:** The primary-group → NATURE mapping is duplicated in `tally_groups.py` and `tally_ledger_master.py`. If you add or rename a primary group, update **both** scripts to keep enrichment fields consistent.
- **Tally version compatibility:** The `FETCH` spec in `tally_ledger_master.py` targets recent Tally Prime builds. Older Tally.ERP 9 versions may not recognise all field names — remove unknown fields from the spec if the API returns an error.
- **Large date ranges:** Even with 900 s read timeout, very large companies exporting a full year may see timeouts. Split into quarterly ranges and concatenate the resulting XMLs if needed.
- **Offline after export:** Once the three master XML files exist, all analysis and slicing scripts work entirely offline — no Tally connection required.
...
*Built for Indian accounting data. Contributions and issue reports welcome.*