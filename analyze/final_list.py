#!/usr/bin/env python3
"""
Ledger names from detect_cross_vouchers, excluding ledgers under selected
group roots (core.groups). Defaults: tally_ledgers_final.xml, daybook XML, tally_groups_final.xml
...
unlocks the manual-review TDS workflow:
  ...
  1. python tds/apply_expense_blocklist.py --input expense_raw.json
     # writes expense_filtered.json + expense_blocklist_report.json
  2. # review expense_blocklist_report.json
  3. python analyze/final_list.py --filtered-expense expense_filtered.json
"""
...
def main() -> None:
    ...
    final_sorted = load_final_ledger_names(
        args.ledgers,
        args.daybook,
        args.groups_xml,
